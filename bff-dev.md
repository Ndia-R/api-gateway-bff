# 🔑 BFF 開発環境ガイド (auth-bff)

本ガイドは、認証ゲートウェイとトークン管理を担う BFF サービス（`auth-bff`）をローカルで開発するための環境設定と利用方法を説明します。BFF はトークン管理のために Redis に依存します。
...
| :------------- | :------------------------------ | :------------------ | :-------------------------------------- | :------- |
| **`auth-bff`** | 認証ゲートウェイ（Spring Boot） | `development`       | ホスト 8888 $\rightarrow$ コンテナ 8080 | `redis`  |
...
- **自動起動と依存性**: `docker-compose.yml` により、`auth-bff` の起動前に **`redis`** コンテナが完全に準備完了（`service_healthy`）になるのを待機します。
...
docker image build --target production -t <レジストリURL>/auth-bff:<タグ> .
```

```bash
# 例
docker image build --target production -t vsv-crystal.skygroup.local/auth-bff:latest .
```
